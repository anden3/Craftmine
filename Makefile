HEADER_PATHS=-iquote Build/Classes -I /usr/local/include -isystem /usr/local/include/freetype2 -isystem /usr/local/opt/icu4c/include
...
COMPILER_FLAGS=everything no-c++98-compat no-c++98-compat-pedantic no-float-equal no-global-constructors \
			   no-exit-time-destructors no-newline-eof no-missing-prototypes no-padded

OBJECTS_FOLDER=Build/Data/Objects
APP_CONTENTS=Build/Craftmine.app/Contents

CPP_FLAGS=-arch x86_64 -std=gnu++11 -F Build -MMD -MT dependencies
DEBUG_FLAGS=-O0 -g
RELEASE_FLAGS=-O3

LINKER_FLAGS=-arch x86_64 -o $(APP_CONTENTS)/MacOS/Craftmine -L Build -F Build -Xlinker -rpath -Xlinker $(APP_CONTENTS)/Frameworks -Xlinker -no_deduplicate -Xlinker -dependency_info
...
all: $(CPP_FILES) $(OBJ_FILES)
	...
	clang++ $(LINKER_FLAGS) -Xlinker $(OBJECTS_FOLDER)/Craftmine_dependency_info.dat -filelist Build/Data/Craftmine.LinkFileList -L /usr/local/lib $(LIBRARY_FLAGS) $(FRAMEWORK_FLAGS)
	\
	...
	for LIB in $(LIBRARIES); do \
		install_name_tool -change /usr/local/lib/lib$$LIB.dylib $(APP_CONTENTS)/Frameworks/lib$$LIB.dylib $(APP_CONTENTS)/MacOS/Craftmine; \
...
$(OBJECTS_FOLDER)/%.o: Classes/%.cpp
	$(info Compiling $<...)
	@clang++ $(addprefix -W,$(COMPILER_FLAGS)) $(CPP_FLAGS) $(HEADER_PATHS) $(DEBUG_FLAGS) -MF $(patsubst %.o,%.d,$@) -c $< -o $@
