            ...
            for (nlohmann::json::iterator it = json.begin(); it != json.end(); ++it) {
                if (it.key() == "name") { block.Name = it.value(); }
                    ...
                    for (unsigned long j = 0; j < 3; j++) {
                        block.Scale[static_cast<int>(j)] = it.value()[j];
                    }
                }

                else if (it.key() == "hitboxOffset") {
                    for (unsigned long j = 0; j < 3; j++) {
                        block.ScaleOffset[static_cast<int>(j)] = it.value()[j];
                    }
                }
                ...
                for (auto &type : json["types"]) {
                    ...
                    for (nlohmann::json::iterator it = type.begin(); it != type.end(); ++it) {
                        if (it.key() == "name") { subType.Name = it.value(); }
                            ...
                            for (unsigned long j = 0; j < 3; j++) {
                                subType.Scale[static_cast<int>(j)] = it.value()[j];
                            }
                        }

                        else if (it.key() == "hitboxOffset") {
                            for (unsigned long j = 0; j < 3; j++) {
                                subType.ScaleOffset[static_cast<int>(j)] = it.value()[j];
                            }
                        }
                            ...
                            for (auto const &element : it.value()) {
                                ...
                                for (int j = 0; j < 3; j++) {
                                    if (startPos[j] < smallestCoord[j]) {
                                        smallestCoord[j] = startPos[j];
                                    }
                                    if (endPos[j] > largestCoord[j]) {
                                        largestCoord[j] = endPos[j];
                                    }
                                }
                                ...
                                for (auto const &texture : element["texCoords"]) {
                                    ...
                                    for (int j = 0; j < 6; j++) {
                                        data.push_back(std::make_pair(startPos + (endPos - startPos) * vertices[index][j],
                                            glm::vec3(texStart + (texEnd - texStart) * tex_coords[index][j], textureID)));
                                    }
